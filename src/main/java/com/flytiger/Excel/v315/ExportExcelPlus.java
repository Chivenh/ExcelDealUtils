...
public class ExportExcelPlus {
	 ...
	 * 不规则批量填充工作表
	 *
	 * @param positions 位置集合 <p>此参数由: {@link #cpsBatch(ExportExcelPlus.CellPosition...)} 方法构造<p/>
	 * @param map 数据集
	 * @author LFH
	 ...
	 * @author LFH
	 */
	public void entryCell(int row, int index, Object value, ExportExcelPlus.Type type) {
	...
	private void entryCellRich(Row rowi, int index, Object value) {
		...
		String v = valuePrepare(value);
		try {
			cell.setCellValue(this.constructorDefine.richTextStringConstructor.newInstance(v));
	 ...
	 * @author LFH
	 */
	private void entryCellInType(Row rowi, int c, Object v, ExportExcelPlus.Type type) {
		type = type == null ? ExportExcelPlus.Type.NORMAL : type;
		switch (type) {
		case NORMAL:
	 ...
	 */
	public Map<Integer, Type> mapType(Map<Integer, Type> type, ExportExcelPlus.Type t, Integer... i) {
		for (Integer index : i) {
			type.put(index, t);
		}
		return type;
	 ...
	 */
	public Map<Integer, Type> mapType(ExportExcelPlus.Type t, Integer... i) {
	 ...
	 */
	public static Map<Integer, Type> mapTypes(Map<Integer, Type> type, ExportExcelPlus.Type t, Integer... i) {
		for (Integer index : i) {
			type.put(index, t);
		}
		return type;
	 ...
	 */
	public static Map<Integer, Type> mapTypes(ExportExcelPlus.Type t, Integer... i) {
		Map<Integer, Type> types = new HashMap<>();
	 ...
	 * 快速创建位置对象
	 *
	 * @return  {@link ExportExcelPlus.CellPosition}
	 * @author LFH
	 * @since 2018年3月7日 下午9:24:16
	 */
	public ExportExcelPlus.CellPosition cps(String key, int row, int cell) {
		return new ExportExcelPlus.CellPosition(key, row, cell);
	}
	 ...
	 */
	public ExportExcelPlus.CellPosition cps(String key, int row, int cell, ExportExcelPlus.Type type) {
		return new ExportExcelPlus.CellPosition(key, row, cell, type);
	}
	 ...
	 * @since 2018年3月11日 下午3:28:05
	 */
	public ExportExcelPlus.CellPosition cps(int row, int cell, Object value) {
		return new ExportExcelPlus.CellPosition(row, cell, value);
	}
	 ...
	 * @since 2018年3月11日 下午3:28:05
	 */
	public ExportExcelPlus.CellPosition cps(int row, int cell, Object value, ExportExcelPlus.Type type) {
		return new ExportExcelPlus.CellPosition(row, cell, value, type);
	}
	 ...
	 *
	 * @param p 位置对象 <div>
	 *       <h3>对象参数p 由 {@link #cps(String, int, int)},{@link #cps(String, int, int, ExportExcelPlus.Type)} 方法创建</h3>
	 *          <div/>
	 * @return {@link ExportExcelPlus.CellPosition}
	 * @author LFH
	 * @since 2018年3月7日 下午9:28:11
	 */
	public List<CellPosition> cpsBatch(ExportExcelPlus.CellPosition... p) {
	 ...
	 *
	 * @param expStyle 样式
	 * @return {@link ExportExcelPlus.ExpStyle}
	 * @author LFH
	 * @since 2018年4月14日 下午9:09:11
	 */
	public ExportExcelPlus.ExpStyle createExpStyle(CellStyle expStyle) {
		return new ExportExcelPlus.ExpStyle(this.theWorkbook, expStyle);
	}
	 ...
	 */
	public ExportExcelPlus.ExpStyle createExpStyle() {
		return new ExportExcelPlus.ExpStyle(this.theWorkbook);
	}
	 ...
	 * @throws Exception
	 */
	public void insertImg(ExportExcelPlus.ImgOption option) throws Exception {
		String type = "";
		...
		// anchor对象创建
		ClientAnchor anchor = this.constructorDefine.clientAnchorConstructor.newInstance (option.getDx1(), option.getDy1(), option.getDx2(),
	 ...
	 * 图片属性
	 */
	public ExportExcelPlus.ImgOption initImgOption(int dx1, int dy1, int dx2, int dy2, int col1, int row1, int col2, int row2,
			String path) {
		return new ExportExcelPlus.ImgOption(dx1, dy1, dx2, dy2, (short) col1, row1, (short) col2, row2, path);
	}
	/**
	 * 图片属性
	 */
	public ExportExcelPlus.ImgOption initImgOption(int dx2, int dy2, int col1, int row1, int col2, int row2, String path) {
		return new ExportExcelPlus.ImgOption(dx2, dy2, (short) col1, row1, (short) col2, row2, path);
	}
	/**
	 * 图片属性
	 */
	public ExportExcelPlus.ImgOption initImgOption(int col1, int row1, int col2, int row2, String path) {
		return new ExportExcelPlus.ImgOption((short) col1, row1, (short) col2, row2, path);
	}
	...
	private class ConstructorDefine{
		private Constructor<? extends RichTextString> richTextStringConstructor;
		private Constructor<? extends ClientAnchor> clientAnchorConstructor;
		...
		ConstructorDefine(Constructor<? extends RichTextString> richTextStringConstructor,
			...
			this.richTextStringConstructor = richTextStringConstructor;
			this.clientAnchorConstructor = clientAnchorConstructor;
		}
	}
	...
	public class CellPosition {
		...
		private ExportExcelPlus.Type type = ExportExcelPlus.Type.NORMAL;// 类型
		...
		private ExportExcelPlus.Type getType() {
			return type;
		}
		...
		private CellPosition(String key, int row, int cell, ExportExcelPlus.Type type) {
			this.key = key;
			this.row = row;
			this.cell = cell;
			this.type = type;
		...
		private CellPosition(int row, int cell, Object value, ExportExcelPlus.Type type) {
			this.row = row;
			this.cell = cell;
			this.value = value;
			this.type = type;
	 ...
	 */
	private enum Align {
		center(CellStyle.ALIGN_CENTER), left(CellStyle.ALIGN_LEFT), right(CellStyle.ALIGN_RIGHT), middle(
				CellStyle.ALIGN_CENTER), top(CellStyle.VERTICAL_TOP), bottom(CellStyle.VERTICAL_BOTTOM);
		private short index;
	...
	public class ExpStyle {
		 ...
		 */
		public ExportExcelPlus.ExpStyle setFontColor(short color) {
		 ...
		 */
		public ExportExcelPlus.ExpStyle setFontSize(int size) {
		 ...
		 */
		public ExportExcelPlus.ExpStyle setAlign(String tAlign, String vAlign) {
			try {
				ExportExcelPlus.Align talign = ExportExcelPlus.Align.valueOf(tAlign);
				this.expStyle.setAlignment(talign.index);
			} catch (Exception e) {
				//TODO
			}
			try {
				ExportExcelPlus.Align valign = ExportExcelPlus.Align.valueOf(vAlign);
		 ...
		 */
		public ExportExcelPlus.ExpStyle setBgColor(short color) {
			this.expStyle.setFillBackgroundColor(color);
			this.expStyle.setFillPattern(CellStyle.SOLID_FOREGROUND);
			return this;
		}
		 ...
		 * @since 2018年4月14日 下午9:11:59
		 */
		public ExportExcelPlus.ExpStyle setFgColor(short color) {
