...
@Deprecated
 public class ExportExcelUtil {
	...
	private HSSFWorkbook tWorkbook;// 当前操作workbook(不能get获取,保证对象结构完善)
	private String rootPath;// web项目根路径.
	private HSSFCellStyle hssfCellStyle;// 只创建一次样式进行复用,以免超过循环创建4000报错,

	private ExportExcelUtil() {

	}

	private ExportExcelUtil(HSSFSheet tSheet, HSSFWorkbook tWorkbook, String rootPath) {
		this.tSheet = tSheet;
		this.tWorkbook = tWorkbook;
		this.rootPath = rootPath;
		this.hssfCellStyle = this.tWorkbook.createCellStyle();
	}
	 ...
	 */
	public static ExportExcelUtil createExportWork(HttpServletRequest request, String path, int at) {
	 ...
	 * @return
	 */
	public static ExportExcelUtil createExportWork(String rootPath, String path, int at) {

		FileInputStream fis = null;
		HSSFSheet tSheet = null;
		HSSFWorkbook wb = null;
		ExportExcelUtil exp = null;
		try {
			fis = fileInput(rootPath, path);
			if (fis != null) {
				wb = new HSSFWorkbook(fis);
				tSheet = wb.getSheetAt(at);
				exp = new ExportExcelUtil(tSheet, wb, rootPath);
			} else {
	...
	public void settSheet(int at) {
		...
		HSSFSheet sheet = this.tWorkbook.getSheetAt(at);
		if (sheet == null) {
	...
	public boolean protectSheet(String password, int... index) {
		...
		try {
			if (index != null && index.length > 0) {
				for (int i : index) {
					HSSFSheet xSheet = this.tWorkbook.getSheetAt(i);
					if (xSheet != null) {
	 ...
	 */
	public HSSFSheet cloneSheet(int at, String... sheetName) {
		...
		if (this.tSheet == null) {
			...
			if (list != null && list.size() > 0) {
				if (rowk < 0) {
					...
					HSSFRow rowi = null;
					HSSFRow rowf = this.entryRow(rowk);
	 ...
	 * @param cellStyle
	 */
	public void entryCell(int row, int index, Object value, HSSFCellStyle cellStyle) {
	 ...
	 * @return
	 */
	public HSSFRow entryRow(int index) {
		HSSFRow rowi = this.tSheet.getRow(index);
		if (rowi == null) {
	 ...
	 * @return
	 */
	public HSSFCell entryCell(int rowi, int celli) {
		HSSFRow row = this.entryRow(rowi);
	...
	public void copyRowStyle(int rowk, int rowi, int s, int e) {
		HSSFRow _rowk = this.entryRow(rowk);
		HSSFRow _rowi = this.entryRow(rowi);
	 ...
	 * @param style
	 */
	public void setRowStyle(int rowi, int s, int e, HSSFCellStyle style) {
		if (this.tSheet == null) {
			System.err.println("请先创建Sheet!");
			return;
		}
		for (; s <= e; s++) {
			HSSFCell cell = entryCell(entryRow(rowi), s);
	 ...
	 * @param times 次数
	 */
	public void setRowStyle(int rowi, int s, int e, HSSFCellStyle style, int times) {
		if (this.tSheet == null) {
			System.err.println("请先创建Sheet!");
			return;
		}
	 ...
	 * @return
	 */
	public HSSFCellStyle getStyle(int rowi, int celli) {
		if (this.tSheet == null) {
			System.err.println("请先创建Sheet!");
			return null;
		}
		HSSFCell cell = this.entryCell(this.entryRow(rowi), celli);
	...
	public void copyCell(int fromRow, int fromCell, int toRow, int toCell, boolean copyValueFlag) {
		...
		HSSFCell s = entryCell(entryRow(fromRow), fromCell);
		HSSFCell t = entryCell(entryRow(toRow), toCell);
	...
	public void copyRow(int fromRow, int toRow, boolean copyValueFlag) {
		...
		HSSFRow f = entryRow(fromRow);
		HSSFRow t = entryRow(toRow);
	...
	public void clearRow(int fromCell, int endCell, int... rowk) {
		...
		for (int i : rowk) {
			HSSFRow rowi = entryRow(i);
			for (; fromCell <= endCell; fromCell++) {
				HSSFCell c = entryCell(rowi, fromCell);
	...
	public int createSheet() {
		HSSFSheet sheet = this.tWorkbook.createSheet();
		return this.tWorkbook.getSheetIndex(sheet);
	}
	...
	public int createSheet(String name) {
		HSSFSheet sheet = this.tWorkbook.createSheet(name);
		return this.tWorkbook.getSheetIndex(sheet);
	}
...
/************************************************************************************************/
	 ...
	 * @param e 结束列
	 */
	private void copyRowStyle(HSSFRow rowk, HSSFRow rowi, int s, int e) {
		if (rowk == null) {
			return;
		}
		HSSFCellStyle style = this.createStyle();
		HSSFCellStyle rowStyle = rowk.getRowStyle();
		if (rowStyle != null) {
	 ...
	 * @return HSSFcell
	 */
	private HSSFCell entryCell(HSSFRow rowi, int index) {
		HSSFCell cell = rowi.getCell(index);
		if (cell == null) {
	 ...
	 * @return
	 */
	private void entryCell(HSSFRow rowi, int index, Object value, HSSFCellStyle style) {
		HSSFCell cell = entryCell(rowi, index);
		cell.setCellStyle(style);
		String v = "";
		if (isNull(value)) {
			v = "";
	 ...
	 * @param value 值
	 */
	private void numCell(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "0";
	 ...
	 * @param value 值
	 */
	private void numCellZ(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "0";
	 ...
	 * @param value
	 */
	private void numCellT(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "0";
	 ...
	 * @return
	 */
	private void entryCell(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "";
	...
	private void entryCellRich(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "";
	 ...
	 * @param
	 */
	private void dateCell(HSSFRow rowi, int index, Date value) {
	 ...
	 * @param
	 */
	private void formulaCell(HSSFRow rowi, int index, Object value) {
		HSSFCell cell = entryCell(rowi, index);
		String v = "";
		if (isNull(value)) {
			v = "";
	 ...
	 */
	private void outFile(HttpServletResponse response, String fname, HSSFWorkbook wb) throws Exception {
		/**********************/
	 ...
	 * @param type 类型
	 */
	private void entryCellInType(HSSFRow rowi, int c, Object v, Type type) {
	 ...
	 *            是否含内容
	 */
	private void copyCell(HSSFCell srcCell, HSSFCell distCell, boolean copyValueFlag) {
	 ...
	 * @param copyValueFlag 是否含内容复制
	 */
	private void copyRow(HSSFRow fromRow, HSSFRow toRow, boolean copyValueFlag) {
...
/**
	...
	public void insertImg(ImgOption option) throws Exception {
		...
		HSSFPatriarch patriarch = this.tSheet.createDrawingPatriarch();
		// anchor对象创建
		HSSFClientAnchor anchor = new HSSFClientAnchor(option.getDx1(), option.getDy1(), option.getDx2(),
		...
		patriarch.createPicture(anchor,
				this.tWorkbook.addPicture(byteArrayOut.toByteArray(), HSSFWorkbook.PICTURE_TYPE_PNG));
	}
	 ...
	 * 
	 * @example
	 * 			HSSFCellStyle style = exp.getStyle(6, 1);<br/>
	 *          exp.entryCell(6, 0).setCellStyle( exp.createExpStyle(style)<br/>
	 *          .setFontColor(HSSFColor.RED.index)<br/>
	 *          .setFgColor(HSSFColor.YELLOW.index)<br/>
	 ...
	 * @return
	 */
	public ExpStyle createExpStyle(HSSFCellStyle expStyle) {
		return new ExpStyle(this.tWorkbook, expStyle);
	}

	/**
	 * @see --示例参看:
	 * @see #createExpStyle(HSSFCellStyle)
	 * 
	 * @author LFH
	 ...
	 */
	private enum Align {
		center(HSSFCellStyle.ALIGN_CENTER), left(HSSFCellStyle.ALIGN_LEFT), right(HSSFCellStyle.ALIGN_RIGHT), middle(
				HSSFCellStyle.ALIGN_CENTER), top(HSSFCellStyle.VERTICAL_TOP), bottom(HSSFCellStyle.VERTICAL_BOTTOM);
		private short index;
	...
	public class ExpStyle {
		private HSSFCellStyle expStyle;
		private HSSFFont expFont;
		private boolean hasFont = false;

		private ExpStyle(HSSFWorkbook work) {
			this.expStyle = work.createCellStyle();
			this.expFont = work.createFont();
		}

		private ExpStyle(HSSFWorkbook work, HSSFCellStyle expStyle) {
		 ...
		 * @param color {@link HSSFColor} 的<b>颜色属性</b> 的 <b>index</b> 属性
		 * @return
		 */
		...
		public ExpStyle setBgColor(short color) {
			...
			this.expStyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
			return this;
		}
